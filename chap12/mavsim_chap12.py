...
DATA = False  # True == show data plotter, false is generally faster to draw
...
while sim_time < 100:
    ...
    input_state = mav.true_state
    # -------path planner - ----
    if path_manager.manager_requests_waypoints is True:
        waypoints = path_planner.update(world_map, input_state, PLAN.R_min)

    # -------path manager-------------
    path = path_manager.update(waypoints, PLAN.R_min, input_state)

    # -------path follower-------------
    autopilot_commands = path_follower.update(path, input_state)

    # -------autopilot-------------
    delta, commanded_state = autopilot.update(autopilot_commands, input_state)
