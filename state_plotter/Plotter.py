...
class Plotter:
    ...
    """
    def __init__(self, update_period=SIM.data_plotting_period,
                 time_window=15, window_title="States"):
        ...
        if not ps2.QtWidgets.QApplication.instance():
            self.app = pg.QtGui.QApplication([])
        else:
            self.app = ps2.QtWidgets.QApplication.instance()
        #self.app = pg.QtGui.QApplication([])
        self.window = pg.GraphicsWindow(title=window_title)
        # self.window.resize(*self.default_window_size)
        self.window.resize(910, 1080)
        self.window.move(910, 0)
    ...
    def update_plots(self):
        ...
        if self.time > self.prev_time or True:
            # Only process data if time has changed
            self.update_counter += 1
            if self.new_data and self.update_counter >= self.update_period:  # only update every x samples
                self.update_counter = 0
