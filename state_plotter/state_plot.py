...
class StatePlot():
    ...
    def update(self):
        ...
        if self.dimension == 1:
            x_data = np.squeeze(np.array(state_objs[0].get_time_vec()))
            y_data = np.squeeze(np.array(state_objs[0].get_data_vec()))
            if self.has_sigma:
                for bound in self.sigma_bounds:
        ...
        if not self.connect:
            ...
            if x_data.shape == () or y_data.shape == ():
                x_data = np.array([x_data.item(0)])
                y_data = np.array([y_data.item(0)])
