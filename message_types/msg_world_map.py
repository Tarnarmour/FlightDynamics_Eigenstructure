...
class MsgWorldMap:
    def __init__(self):
        ...
        self.building_details = np.zeros((PLAN.num_blocks**2, 3))
        for i in range(PLAN.num_blocks):
            for j in range(PLAN.num_blocks):
                north = self.building_north[i]
                east = self.building_east[j]
                height = self.building_height[i, j]
                self.building_details[i * PLAN.num_blocks + j] = np.array([north, east, height])
