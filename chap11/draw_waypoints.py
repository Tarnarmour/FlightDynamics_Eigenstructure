...
class DrawWaypoints:
    def __init__(self, waypoints, radius, color, window):
        ...
        if waypoints.type=='straight_line' or waypoints.type=='fillet':
            points = self.straight_waypoint_points(np.asarray(waypoints.ned))
        elif waypoints.type=='dubins':
            points = self.dubins_points(waypoints, self.radius, 0.1)
        waypoint_color = np.tile(color, (len(points), 1))
    ...
    def straight_waypoint_points(self, p):
        R = np.array([[0, 1, 0], [1, 0, 0], [0, 0, -1]])
        # for i, p in enumerate(waypoints.ned):
        #     waypoints.ned[i] = R @ p
        points = R @ p.T
        return points.T
        return waypoints.ned
